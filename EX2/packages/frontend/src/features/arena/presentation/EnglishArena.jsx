  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 md:p-12 pb-24 md:pb-12 flex flex-col items-center transition-colors duration-300" dir="rtl">
      ...
      <div className="w-full max-w-4xl flex flex-col-reverse sm:flex-row justify-between items-center gap-6 mb-10">
        ...
        <h1 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white">זירת אימון באנגלית</h1>
      </div>

      {error && (
        <div className="w-full max-w-4xl p-4 bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/30 text-rose-600 dark:text-rose-450 rounded-2xl text-center font-bold mb-6">
          {error}
        </div>
      )}
      ...
      {status === "disconnected" ? (
        /* Room Selector UI */
        <div className="w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-8 md:p-10 shadow-sm flex flex-col items-center text-center gap-8 animate-fade-in transition-colors duration-300">
          <div className="w-20 h-20 bg-indigo-50 dark:bg-indigo-950/20 rounded-full flex items-center justify-center text-indigo-500 dark:text-indigo-400 shadow-inner">
            <Users className="w-10 h-10" />
          </div>
          <div>
            <h2 className="text-3xl font-extrabold text-slate-800 dark:text-white">בחרו חדר שיחה</h2>
            <p className="text-slate-500 dark:text-slate-400 text-md mt-2">הצטרפו לחדר שיחה כללי או התחברו עם חבר בחדר פרטי בעזרת קוד.</p>
          </div>
          ...
          <div className="w-full flex flex-col sm:flex-row gap-6 mt-4">
            ...
            <form onSubmit={handleJoinCustom} className="flex-1 flex flex-col gap-3">
              <input
                ...
                className="w-full px-5 py-3.5 border border-slate-200 dark:border-slate-700 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-white text-center text-lg transition-colors"
                required
              />
              <button
                type="submit"
                className="w-full py-3 bg-white dark:bg-slate-900 border-2 border-indigo-200 dark:border-indigo-800 hover:bg-indigo-50 dark:hover:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400 font-bold rounded-2xl transition-all text-lg cursor-pointer"
              >
                כנס לחדר פרטי
              </button>
            </form>
        ...
        <div className="w-full max-w-3xl flex flex-col gap-8 animate-fade-in">
          {/* Status card */}
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-8 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-4 transition-colors duration-300">
            <div>
              <span className="text-sm font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">חדר פעיל: {roomId}</span>
            ...
            <div className="flex gap-4">
              <button
                onClick={toggleMute}
                className={`p-4 rounded-full shadow-md transition-all cursor-pointer ${isMuted ? "bg-rose-500 text-white animate-pulse" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-350 hover:bg-slate-200 dark:hover:bg-slate-700"}`}
          ...
          <div className="bg-gradient-to-br from-indigo-500 to-purple-600 text-white rounded-[2.5rem] p-10 shadow-lg flex flex-col items-center text-center justify-between min-h-[300px]">
            ...
            <div className="my-6">
              <h4 className="text-2xl md:text-3xl font-serif italic leading-relaxed" dir="ltr">
